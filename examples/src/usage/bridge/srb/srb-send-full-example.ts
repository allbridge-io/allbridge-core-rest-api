...
const main = async () => {
  ...
  const sourceToken = ensure(chains['SRB'].tokens.find((t: any) => t.symbol == "USDC"));
  const destinationToken = ensure(chains['BSC'].tokens.find((t: any) => t.symbol == "USDT"));
  ...
  const xdrTx: string = (await axios.get(
  ...
  const transaction = TransactionBuilder.fromXDR(xdrTx, sorobanNetworkPassphrase);
  ...
  try {
    ...
    if (restoreXdrResp.status == 200 && restoreXdrResp.data) {
      const restoreXdrTx = TransactionBuilder.fromXDR(restoreXdrResp.data, sorobanNetworkPassphrase) as Transaction;
      restoreXdrTx.sign(srbKeypair);
      const signedRestoreXdrTx = restoreXdrTx.toXDR();
      const transaction = TransactionBuilder.fromXDR(signedRestoreXdrTx, sorobanNetworkPassphrase) as Transaction;
      ...
      let counter = 10
      while (counter > 0) {
        const getTransactionResponse = await server.getTransaction(sentRestoreXdrTx.hash);
        confirmRestoreXdrTx = getTransactionResponse;
        if (getTransactionResponse.status === SorobanRpc.Api.GetTransactionStatus.FAILED
          || getTransactionResponse.status === SorobanRpc.Api.GetTransactionStatus.SUCCESS) {
          break;
        }
        counter--;
      }
      if (!!confirmRestoreXdrTx && confirmRestoreXdrTx.status === SorobanRpc.Api.GetTransactionStatus.NOT_FOUND) {
        console.log(
          `Waited for Restore transaction to complete, but it did not. ` +
          `Check the transaction status manually. ` +
          `Hash: ${sentRestoreXdrTx.hash}`
        );
      } else if (!!confirmRestoreXdrTx && confirmRestoreXdrTx.status === SorobanRpc.Api.GetTransactionStatus.FAILED) {
  ...
  const tx = TransactionBuilder.fromXDR(signedTx, sorobanNetworkPassphrase) as Transaction;
  ...
  if (!!confirm && confirm.status === SorobanRpc.Api.GetTransactionStatus.NOT_FOUND) {
    console.log(
      `Waited for transaction to complete, but it did not. ` +
      `Check the transaction status manually. ` +
      `Hash: ${sent.hash}`
    );
  } else if (!!confirm && confirm.status === SorobanRpc.Api.GetTransactionStatus.FAILED) {
  ...
  const balanceLine = (await axios.get(
